...
import { addGuestBasketItem, saveDirectCheckout } from "@/lib/guestBasket";
    ...
    <main className="container py-8 sm:py-12">
      ...
      <section className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.06fr)_minmax(23rem,.94fr)] lg:gap-14">
        ...
        <article className="lg:pt-1">
          ...
          <div className="mt-6 grid gap-4 border-y border-slate-200 py-5 sm:grid-cols-[auto_1fr] sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[.13em] text-slate-500">Quantity</p><div className="mt-2 flex items-center gap-3"><QuantityControl value={quantity} max={Math.max(1, product.stockQuantity)} onChange={setQuantity} /><span className="text-xs leading-5 text-slate-500">Choose up to the listed stock.</span></div></div><div className="grid gap-3 sm:grid-cols-2"><Button className="h-12 rounded-none bg-slate-950 text-white hover:bg-[#b88d3c]" disabled={product.stockQuantity < 1} onClick={() => { addGuestBasketItem({ productId: product.id, slug: product.slug, name: product.name, sku: product.sku, price: Number(product.price), quantity, imageUrl: product.imageUrl || productImage(product) }); toast.success(`Added ${quantity} item${quantity === 1 ? "" : "s"} to your guest basket.`); }}><PackageCheck className="mr-2 h-4 w-4" />{product.stockQuantity < 1 ? "Currently unavailable" : "Add to basket"}</Button><Button variant="outline" className="h-12 rounded-none border-[#b88d3c] bg-[#fffaf0] text-[#694914] hover:bg-[#f4e4b8] hover:text-[#3e2b08]" disabled={product.stockQuantity < 1} onClick={() => { saveDirectCheckout({ productId: product.id, slug: product.slug, name: product.name, sku: product.sku, price: Number(product.price), quantity, imageUrl: product.imageUrl || productImage(product) }); toast.success("Opening your local test checkout."); setLocation("/checkout"); }}><ArrowRight className="mr-2 h-4 w-4" />{product.stockQuantity < 1 ? "Unavailable" : "Buy now"}</Button></div></div>
          ...
          <p className="mt-3 text-center text-xs leading-5 text-slate-500">Buy now opens a browser-local test checkout for this item only. No payment is taken and no delivery is created.</p>
